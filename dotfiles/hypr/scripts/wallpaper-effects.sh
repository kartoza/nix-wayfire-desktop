...
if [ $1 == "reload" ]; then
    ...
    options="$(ls $(xdg-config-resolve hypr/effects/wallpaper/))\noff"

    # Open rofi
    choice=$(echo -e "$options" | rofi -dmenu -replace -config $(xdg-config-resolve rofi/config-themes.rasi) -i -no-show-icons -l 5 -width 30 -p "Hyprshade")
    if [ ! -z $choice ]; then
        echo "$choice" >$(xdg-config-resolve ml4w/settings/wallpaper-effect.sh)
