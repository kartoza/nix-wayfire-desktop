...
if [ $1 == "reload" ]; then
    ...
    options="$(ls /etc/xdg/hypr/effects/wallpaper/)\noff"

    # Open rofi
    choice=$(echo -e "$options" | rofi -dmenu -replace -config /etc/xdg/rofi/config-themes.rasi -i -no-show-icons -l 5 -width 30 -p "Hyprshade")
    if [ ! -z $choice ]; then
        echo "$choice" >/etc/xdg/ml4w/settings/wallpaper-effect.sh
